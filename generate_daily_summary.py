...
_SUMMARY_STYLE        = str(_cfg.get("daily_summary_style")        or _cfg.get("summary_style",        "narrative")).lower()
_SUMMARY_WORD_LIMIT   = int(_cfg.get("daily_summary_word_limit")   or _cfg.get("summary_word_limit",   130))
_SUMMARY_BULLET_COUNT = int(_cfg.get("daily_summary_bullet_count") or _cfg.get("summary_bullet_count", 5))
...
try:
    for item in gh_get(
        "https://api.github.com/search/issues",
        {"q": f"author:{GITHUB_ACTOR} is:issue created:{date_str}..{date_str}"},
    ):
        if item["html_url"] in _seen_issue_urls:
            continue
        repo = item.get("repository_url", "").replace("https://api.github.com/repos/", "")
...
def generate_narrative(prs, commits, branch_work, created_issues=None, pr_reviews=None):
    ...
    if _SUMMARY_STYLE == "bullets":
        prompt = (
            ...
            f"{activity_text}\n\n"
            f"Write exactly {_SUMMARY_BULLET_COUNT} concise bullet points summarising the key tasks — omit the subject pronoun and start each bullet directly with a past-tense verb (e.g. 'Worked on...', not 'I worked on...'). "
...
def build_pr_table(prs):
    ...
    for p in prs:
        ...
        rows.append(
            f"| [#{p['number']}]({p['url']}) | {p['repo']} | {p['title']} | {status_badge(state_key)} |"
        )
    return "\n".join(rows) + "\n"
...
# ── Build output ──────────────────────────────────────────────────────────────
narrative    = generate_narrative(all_prs, commit_messages, branch_work_commits, created_issues, pr_reviews)
